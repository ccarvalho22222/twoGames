...
public class SecurityBotBehaviour : MonoBehaviour {
    ...
    void shoot()
    {
        ...
        bulletStartX = Gun.transform.position.x + 1.1f * Mathf.Cos(GunAngle);
        bulletStartY = Gun.transform.position.y + 1.1f * Mathf.Sin(GunAngle);

        GameObject tempBullet = (GameObject)Instantiate(Bullet, new Vector3(bulletStartX, bulletStartY), Bullet.transform.rotation);
        tempBullet.SetActive(true);

        tempBullet.GetComponent<SecurityBotBulletBehavior>().bulletVelocity = new Vector3(bulletSpeed * Mathf.Cos(GunAngle) + rB.velocity.x,
