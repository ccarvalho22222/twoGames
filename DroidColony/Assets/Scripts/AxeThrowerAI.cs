...
public class AxeThrowerAI : MonoBehaviour {
    ...
    void shoot()
    {
        ...
        bulletStartX = Gun.transform.position.x + bulletOffset * Mathf.Cos(GunAngle);
        bulletStartY = Gun.transform.position.y + bulletOffset * Mathf.Sin(GunAngle);

        GameObject tempBullet = (GameObject)Instantiate(Bullet, new Vector3(bulletStartX, bulletStartY), Bullet.transform.rotation);
        tempBullet.SetActive(true);

        tempBullet.GetComponent<ThrowingAxeMovement>().bulletVelocity = new Vector3(bulletSpeed * Mathf.Cos(GunAngle) + rB.velocity.x,
