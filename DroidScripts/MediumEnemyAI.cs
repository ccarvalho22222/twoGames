...
public class MediumEnemyAI : MonoBehaviour {
	...
	void Update () {
        ...
        if (GetComponent<EnemyEnabledScript>().engaged)
        {
            ...
            {
                if (Mathf.Abs(spikeAngle - mouseAngle) < Mathf.PI / 16)
                {
                    dash();
                    dashing = true;
                    dashCooldownCounter = 0;
                    AudioSource.PlayClipAtPoint(enemyDash, transform.position);
                }
            }
            else
            {
            ...
            if (dashing == true)
            {
                Spike.GetComponent<EnemySpikeBehavior>().damage = dashDamage;
                if (dashTimeCounter++ > dashTime)
                {
                    dashing = false;
                    Spike.GetComponent<EnemySpikeBehavior>().damage = 0;
                    dashTimeCounter = 0;
                }
            }
            else
