...
public class SprayBotBehaviour : MonoBehaviour
{
    ...
    private float shootStallTime = 1;
    private float shootStallCounter = 0;

    public float shotCooldownTime;
    public float sprayTime;
    private float sprayTimeCounter;
    ...
    void Update()
    {
        ...
        if (engaged)
        {
            ...
            if (shotCooldownCounter < shotCooldownTime)
            {
                shotCooldownCounter++;
            ...
            {
                if (sprayTimeCounter < sprayTime)
                {
                    if(shootStallCounter++ > shootStallTime)
                    {
                        shoot();
                        shootStallCounter = 0;
                    }
    ...
    void shoot()
    {
        ...
        tempBullet.GetComponent<SprayBotBulletBehaviour>().bulletVelocity = new Vector3(bulletSpeed * Mathf.Cos(GunAngle) + rB.velocity.x,
                                                                        bulletSpeed * Mathf.Sin(GunAngle) + rB.velocity.y);

        tempBullet = (GameObject)Instantiate(Bullet, new Vector3(bulletStartX, bulletStartY), Bullet.transform.rotation);
        tempBullet.SetActive(true);

        tempBullet.GetComponent<SprayBotBulletBehaviour>().bulletVelocity = new Vector3(bulletSpeed * Mathf.Cos(GunAngle) + rB.velocity.x,
                                                                        bulletSpeed * Mathf.Sin(GunAngle - (Mathf.PI/16)) + rB.velocity.y);
